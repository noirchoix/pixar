<script lang="ts">
  ...
  let customNotes = '';
  let customRuleInput =
    'Use active verbs rather than nouns and cut clutter between subject and main verb.';
  ...
  async function exportTxt() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.message || 'Failed to export script.');
      }

      if (data.downloadUrl) {
        exportResult = data;
        window.location.href = data.downloadUrl;
      } else if (data.inlineContent) {
        const blob = new Blob([data.inlineContent], {
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Structured Pixar-style script generation with flow confirmation, scoring, and text export."
  />
</svelte:head>
...
<div class="page">
  <section class="hero">
    <div>
      ...
      <p class="lede">
        Build short animated scripts through a disciplined pipeline: topic, contradiction, beat
        flow, confirmation, script, scoring, and text export.
      </p>
    </div>
    <div class="hero-card">
  ...
  <section class="panel form-panel">
    ...
    <label>
      ...
      <textarea
        bind:value={topic}
        rows="3"
        placeholder="Example: a tiny cloud afraid of raining on the town it loves"
      ></textarea>
    </label>
    ...
    <label>
      ...
      <textarea
        ...
        rows="3"
        placeholder="Optional direction such as more visual, gentler ending, stronger humor"
      ></textarea>
    </label>
    ...
    <section class="panel">
      ...
      <div class="stats">
        ...
        <div>
          ...
          ><strong>{flow.targetWordRange[0]} - {flow.targetWordRange[1]}</strong>
        </div>
      </div>
        ...
        <div class="grid">
          ...
          <label>
            ...
            <input
              ...
              placeholder="make the ending gentler, add more wonder, reduce dialogue"
            />
          </label>
        </div>
  ...
  {#if scriptResult}
    <section class="panel">
      ...
      <div class="stats">
        ...
        <div>
          <span class="label">Word count</span><strong>{scriptResult.estimatedWordCount}</strong>
        </div>
      ...
      <div class="score-grid">
        <div class="mini-card">
          <span>Emotional clarity</span><strong>{scriptResult.score.emotionalClarity}</strong>
        </div>
        <div class="mini-card">
          <span>Causal coherence</span><strong>{scriptResult.score.causalCoherence}</strong>
        </div>
        <div class="mini-card">
          <span>Visualizability</span><strong>{scriptResult.score.visualizability}</strong>
        </div>
        <div class="mini-card">
          <span>Character agency</span><strong>{scriptResult.score.characterAgency}</strong>
        </div>
        <div class="mini-card">
          <span>Dialogue</span><strong>{scriptResult.score.dialogueNaturalness}</strong>
        </div>
        <div class="mini-card">
          <span>Rule compliance</span><strong>{scriptResult.score.ruleCompliance}</strong>
        </div>
      </div>
      ...
      {#if exportResult}
        <p class="download">
          ...
          {#if exportResult.downloadUrl}
            <a href={exportResult.downloadUrl}>{exportResult.fileName}</a>
          {:else}
